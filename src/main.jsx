...
const Background = () => {
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      particlesRef.current = Array.from({ length: count }, () => ({
        ...
        r: Math.random() * 2 + 0.6, opacity: Math.random() * 0.22 + 0.06,
    ...
    const draw = (time) => {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (dist < 150) {
            ctx.strokeStyle = `rgba(153,27,27,${(1 - dist / 150) * 0.07})`;
      ...
      if (mx > 0 && my > 0) {
        ...
        pts.forEach(p => {
          ...
          if (dist < 200) { ctx.strokeStyle = `rgba(153,27,27,${(1 - dist / 200) * 0.15})`; ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(mx, my); ctx.stroke(); }
        });
      }

      pts.forEach(p => {
        const pulse = Math.sin(time * p.pulseSpeed + p.pulsePhase) * 0.5 + 0.5;
        ctx.fillStyle = `rgba(153,27,27,${p.opacity + pulse * 0.07})`;
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r + pulse * 0.5, 0, Math.PI * 2); ctx.fill();
      });
  ...
  return (
    <>
      <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundImage: `url(${MAP_BG_SRC})`, backgroundSize: "cover", backgroundPosition: "center right", opacity: 0.55, pointerEvents: "none", zIndex: 0 }} />
